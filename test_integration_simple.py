...
def test_basic_integration():
    """测试基本集成"""
    print("[TEST] 测试 Orchestrator 与 AgentRunner 集成...")
    ...
    try:
        ...
        print("[SUCCESS] AgentRunner 导入成功")

        # 测试 Orchestrator 导入
        from src.orchestrator.orchestrator import Orchestrator
        from src.orchestrator.config import OrchestratorConfig
        print("[SUCCESS] Orchestrator 导入成功")

        # 测试配置加载
        config = OrchestratorConfig()
        print(f"[SUCCESS] 配置加载成功: {config.agent.mode} 模式")
        ...
        print(f"[SUCCESS] AgentRunner 初始化成功: {config.agent.path}, {config.agent.mode} 模式")

        # 测试 Orchestrator 初始化
        orchestrator = Orchestrator(config)
        print("[SUCCESS] Orchestrator 初始化成功")
        ...
        print("[SUCCESS] TaskParams 创建成功")

        print("\n[SUCCESS] 集成测试通过！Orchestrator 与 Agent 的接口正常工作。")
        return True
...
def test_agent_runner_modes():
    ...
    for mode in modes:
        try:
            ...
            print(f"[SUCCESS] {mode} 模式实例化成功")

        except Exception as e:
            print(f"[ERROR] {mode} 模式失败: {e}")

if __name__ == "__main__":
    print("[START] 开始测试 Orchestrator-Agent 集成...")

    success = test_basic_integration()
    test_agent_runner_modes()

    if success:
        print("\n[SUCCESS] 所有测试通过！Agent 调用机制已正确实现。")
    else:
        print("\n[ERROR] 测试失败，请检查实现。")
        sys.exit(1)