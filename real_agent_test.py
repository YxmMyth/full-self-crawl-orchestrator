#!/usr/bin/env python3
"""
真实测试：orchestrator调用实际agent访问CodePen寻找HTML格式PPT
"""
...
async def run_real_test():
    ...
    print("🚀 开始真实测试：orchestrator调用实际agent")
    print("目标：寻找HTML格式的PPT资源")
    print("="*60)

    # 创建配置 - 使用subprocess模式连接到实际的agent
    config = OrchestratorConfig()

    # 更新配置指向实际的agent路径
    config.agent.path = "../full-self-crawl-agent"
    config.agent.mode = "subprocess"  # 使用subprocess模式连接到实际agent
    config.site_discovery.min_sites = 1
    config.site_discovery.max_sites = 2  # 限制站点数量
    config.scheduler.mode = "serial"  # 使用串行模式
    config.scheduler.agent_timeout_min = 30  # 设置较长超时时间

    print(f"📁 Agent路径: {config.agent.path}")
    print(f"⚙️ Agent模式: {config.agent.mode}")
    print(f"⏱️ 超时设置: {config.scheduler.agent_timeout_min}分钟")
    print()
...
if __name__ == "__main__":
    ...
    try:
        if hasattr(asyncio, 'run'):
            ...
            loop.run_until_complete(run_real_test())
