...
from orchestrator.models import RefinedRequirement, CandidateSite, TaskResult, TaskParams, ResearchResult, SiteRanking


def simple_format_result(result):
    """简单的结果格式化，避免使用可能包含表情符号的函数"""
    lines = []
    lines.append(f"Query: {result.query}")
    lines.append(f"Task ID: {result.task_id}")
    lines.append(f"Total sites: {result.total_sites}")
    lines.append(f"Successful sites: {result.successful_sites}")
    lines.append(f"Failed sites: {result.failed_sites_count}")

    if result.rankings:
        lines.append("Ranked results:")
        for ranking in result.rankings:
            lines.append(f"  - Rank {ranking.rank}: {ranking.site_name} ({ranking.site_url})")
            lines.append(f"    Quality: {ranking.quality_score}, Records: {ranking.total_records}")

    return "\\n".join(lines)


async def run_pure_text_demo():
    """运行纯文本演示"""

    print("orchestrator与agent集成测试 - 纯文本版本")
    print("=" * 50)
    print()
    ...
    # 创建orchestrator实例（通过环境变量设置不显示表情符号）
    import os
    ...
    summary = simple_format_result(research_result)
    print("  最终摘要:")
    print("  " + "\\n  ".join(summary.split("\\n")[:10]))  # 只显示前10行
    print("  ...")
    ...
    print()
    print("总结:")
    print("- orchestrator成功接收用户需求: '帮我找HTML格式的PPT'")
...
if __name__ == "__main__":
    asyncio.run(run_pure_text_demo())